...
export function QiblaCompass({
  ...
  if (!mounted || !qiblaInfo) {
    return (
      <div
        className={cn(
          "flex flex-col items-center p-6 rounded-3xl bg-[#022c22]/40 backdrop-blur-xl border border-[#D4AF37]/20",
          className,
        )}
      >
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center p-4 md:p-6 rounded-[2.5rem] transition-all duration-700",
        isAligned
          ? "bg-[#064e3b]/60 border-emerald-500/40 shadow-[0_0_50px_rgba(16,185,129,0.1)]"
          : "bg-[#022c22]/40 border-[#D4AF37]/20",
        className,
      )}
    >
      ...
      {/* Compass UI */}
      <div className="relative w-36 h-36 mb-4 md:mb-6 md:w-48 md:h-48 group">
        {/* Fixed Outer Frame */}
        <div
          className={cn(
        ...
        >
          ...
          {[...Array(8)].map((_, i) => (
            <div
              ...
              style={{
                transform: `translateX(-50%) rotate(${i * 45}deg) translateY(4px)`,
              }}
            />
          ))}
          ...
          <div className="absolute inset-3 rounded-full bg-[#022c22] border border-[#D4AF37]/10 flex items-center justify-center shadow-2xl">
            ...
            <div
              className={cn(
                "absolute top-2 font-black text-sm",
                isAligned ? "text-emerald-400" : "text-[#D4AF37]",
              )}
            >
              N
            </div>
            <div className="absolute right-2 font-black text-sm text-white/10">
              E
            </div>
            <div className="absolute bottom-2 font-black text-sm text-white/10">
              S
            </div>
            <div className="absolute left-2 font-black text-sm text-white/10">
              W
            </div>
            ...
            <div
              className={cn(
                "text-3xl z-20 transition-transform duration-500",
                isAligned ? "scale-110" : "",
              )}
            >
              🕋
      ...
      {/* Quick Stats */}
      <div className="grid grid-cols-2 gap-2 md:gap-4 w-full mb-3 md:mb-4">
        <div className="flex flex-col items-center p-2 md:p-3 bg-white/5 rounded-2xl border border-white/5">
        ...
        <div className="flex flex-col items-center p-2 md:p-3 bg-white/5 rounded-2xl border border-white/5">
          ...
          <span className="text-xs md:text-sm font-bold text-[#D4AF37]">
            {qiblaInfo.direction}° {compassDirection}
          </span>
        </div>
      </div>

      {/* Manual Adjustment Section */}
      <div className="w-full space-y-2 pt-3 border-t border-white/10 animate-fade-in">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Navigation className="w-3.5 h-3.5 text-[#D4AF37]" />
            <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">
              {t.qibla.manualAdjustment}
            </span>
          </div>
        ...
        {(isStaticMode || isManualMode) && (
          <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            />
            <div className="hidden md:flex justify-between text-[8px] font-black text-white/10 uppercase tracking-widest">
              <span>N</span>
              <span>E</span>
              <span>S</span>
              <span>W</span>
      ...
      {!permissionGranted && (
        <button
          ...
          className="mt-3 px-6 py-2 text-xs bg-[#D4AF37] text-black font-black rounded-xl hover:scale-105 transition-transform uppercase tracking-widest"
        >
          {t.qibla.enableNow}
        </button>
      )}
